...
public class JVoxSlotItemHandler<I> extends SlotItemHandler implements ISlotItemHandler<I>
{
    ...
    public JVoxSlotItemHandler(
      final com.ldtteam.jvoxelizer.item.handling.IItemHandler itemHandler,
      final int index,
      final int xPosition,
      final int yPosition,
      final I instanceData,
      final ForgeSlotItemHandlerPipeline<ISlotItemHandler<I>, I> pipeline)
    {
        super(ItemHandler.asForge(itemHandler), index, xPosition, yPosition);
    ...
    public ItemStack onTake(final EntityPlayer thePlayer, final ItemStack stack)
    {
        return asForge(
          PipelineProcessor.processTypedPipeline(
            ...
            (c) -> fromForge(super.onTake(PlayerEntity.asForge(c.getThePlayer()), asForge(c.getStack())))
          )
        );
    }
    ...
    public boolean isHere(final net.minecraft.inventory.IInventory inv, final int slotIn)
    {
        return PipelineProcessor.processTypedPipeline(
          this,
          new IsHereContext(Inventory.fromForge(inv), slotIn),
          pipeline.getIsHerePipeline(),
          (c) -> super.isHere(Inventory.asForge(c.getInv()), c.getSlotIn())
        );
    }
    ...
    {
        return ItemHandler.asForge(
          PipelineProcessor.processTypedPipeline(
            this,
            new GetItemHandlerContext(),
            pipeline.getGetItemHandlerPipeline(),
            (c) -> ItemHandler.fromForge(super.getItemHandler())
          )
        );
    }

    @Override
    public com.ldtteam.jvoxelizer.item.handling.IItemHandler getHandler()
    {
        return ItemHandler.fromForge(this.getItemHandler());
    }
    ...
    {
        return fromForge(this.onTake(PlayerEntity.asForge(thePlayer), asForge(stack)));
    }
    ...
    @Override
    public boolean isHere(final com.ldtteam.jvoxelizer.item.handling.IInventory inv, final int slotIn)
    {
        return this.isHere(Inventory.asForge(inv), slotIn);
    }
