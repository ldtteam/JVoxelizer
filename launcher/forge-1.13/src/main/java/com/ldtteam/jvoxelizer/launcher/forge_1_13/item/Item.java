...
import com.ldtteam.jvoxelizer.launcher.forge_1_13.enchantment.Enchantment;
import com.ldtteam.jvoxelizer.launcher.forge_1_13.entity.Entity;
import com.ldtteam.jvoxelizer.launcher.forge_1_13.entity.ai.AttributeModifier;
...
import com.ldtteam.jvoxelizer.launcher.forge_1_13.entity.passive.HorseArmorType;
...
import com.ldtteam.jvoxelizer.launcher.forge_1_13.util.actionresult.ActionResult;
...
public class Item implements IItem<DummyInstanceData>, IForgeJVoxelizerWrapper
{
    ...
    public IActionResultType onItemUse(
      ...
      final float hitZ)
    {
        return ActionResultType.fromForge(forgeItem.onItemUse(new ItemUseContext(
          PlayerEntity.asForge(player),
          PlayerEntity.asForge(player).getHeldItem(Hand.asForge(hand)),
    ...
    {
        return forgeItem.getTranslationKey(ItemStack.asForge(stack));
    }

    @Override
    public String getUnlocalizedName()
    {
        return forgeItem.getTranslationKey();
    }
    ...
    {
        forgeItem.inventoryTick(ItemStack.asForge(stack), Dimension.asForge(worldIn), Entity.asForge(entityIn), itemSlot, isSelected);
    }
    ...
    {
        return forgeItem instanceof ItemMap;
    }

    @Override
    public IActionType getItemUseAction(final IItemStack stack)
    {
        return ActionType.fromForge(forgeItem.getUseAction(ItemStack.asForge(stack)));
    }

    @Override
    public int getMaxItemUseDuration(final IItemStack stack)
    {
        return forgeItem.getUseDuration(ItemStack.asForge(stack));
    }
    ...
    {
        return forgeItem.getDisplayName(ItemStack.asForge(stack)).getFormattedText();
    }
    ...
    {
        return ItemGroup.fromForge(forgeItem.getGroup());
    }
    ...
    {
        return forgeItem.getAttributeModifiers(EquipmentSlot.asForge(equipmentSlot)).entries().stream()
    ...
    {
        return NBTCompound.fromForge(forgeItem.getShareTag(ItemStack.asForge(stack)));
    }

    @Override
    public void readNBTShareTag(final IItemStack stack, final INBTCompound nbt)
    {
        forgeItem.readShareTag(ItemStack.asForge(stack), NBTCompound.asForge(nbt));
    }
    ...
    {
        return forgeItem.onEntityItemUpdate(ItemStack.asForge(entityItem.getItemStack()), ItemStackEntity.asForge(entityItem));
    }

    @Override
    public IItemGroup<?>[] getItemGroups()
    {
        return forgeItem.getCreativeTabs().stream().map(ItemGroup::fromForge).toArray(IItemGroup[]::new);
    }
    ...
    {
        forgeItem.onArmorTick( ItemStack.asForge(iItemStack), Dimension.asForge(iDimension), PlayerEntity.asForge(player));
    }

    @Override
    public boolean isValidArmor(final IItemStack stack, final IEquipmentSlot armorType, final IEntity iEntity)
    {
        return forgeItem.canEquip(ItemStack.asForge(stack), EquipmentSlot.asForge(armorType), Entity.asForge(iEntity));
    }
    ...
    {
        return forgeItem.onEntitySwing(ItemStack.asForge(stack), LivingBaseEntity.asForge(entityLiving));
    }
    ...
    {
        return forgeItem.canHarvestBlock(ItemStack.asForge(stack), BlockState.asForge(state));
    }
    ...
    {
        throw new FeatureNotImplementedException("Forge 1.13 does not support setting the harvest level after construction.");
    }

    @Override
    public Set<String> getToolClasses(final IItemStack stack)
    {
        throw new FeatureNotImplementedException("Forge 1.13 does not support setting the tool class after construction.");
    }
    ...
    {
        return forgeItem.getHarvestLevel(ItemStack.asForge(stack), ToolType.get(toolClass), PlayerEntity.asForge(player), BlockState.asForge(blockState));
    }
    ...
    {
        return forgeItem.getBurnTime(ItemStack.asForge(stack));
    }
    ...
    {
        forgeItem.onHorseArmorTick(ItemStack.asForge(armor), Dimension.asForge(world), LivingEntity.asForge(horse));
    }
    ...
    {
        return forgeItem.getTranslationKey(ItemStack.asForge(pItemStack1));
    }
    ...
    public void getSubItems(IItemGroup<?> tab, List<IItemStack> items)
    {
        ...
        forgeItem.fillItemGroup(ItemGroup.asForge(tab), nonNullList);
    ...
    public void addInformation(final IItemStack stack, final IDimension worldIn, final List<String> tooltip, final IToolTipFlag flagIn)
    {
        ...
        forgeItem.addInformation(ItemStack.asForge(stack), Dimension.asForge(worldIn), textComponents, ToolTipFlag.asForge(flagIn));

        tooltip.clear();
        tooltip.addAll(textComponents.stream().map(ITextComponent::getFormattedText).collect(Collectors.toList()));
    }
