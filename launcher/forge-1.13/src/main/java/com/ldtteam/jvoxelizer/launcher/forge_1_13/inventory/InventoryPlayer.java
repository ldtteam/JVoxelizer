...
import com.ldtteam.jvoxelizer.launcher.forge_1_13.item.Item;
...
import com.ldtteam.jvoxelizer.launcher.forge_1_13.util.nbt.NBTList;
...
public class InventoryPlayer extends Inventory implements IInventoryPlayer
{
    ...
    {
        return forgeInventoryPlayer.clearMatchingItems((itemstack -> (itemIn == null || itemstack.getItem() == itemIn) && (itemNBT == null || NBTUtil.areNBTEquals(NBTCompound.asForge(itemNBT), itemstack.serializeNBT(), true))), removeCount);
    }
    ...
    {
        return NBTList.fromForge(forgeInventoryPlayer.write(new NBTTagList()));
    }

    @Override
    public void readFromNBT(final INBTList nbtTagListIn)
    {
        forgeInventoryPlayer.read(NBTList.asForge(nbtTagListIn));
    }
