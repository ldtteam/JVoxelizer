...
public class KeyboardListener {
   ...
   public void onKeyEvent(long windowPointer, int key, int scanCode, int action, int modifiers) {
      if (windowPointer == this.mc.mainWindow.getHandle()) {
         ...
         if ((!(this.mc.currentScreen instanceof GuiControls) || ((GuiControls)iguieventlistener).time <= Util.milliTime() - 20L)) {
            if (action == 1) {
            ...
            } else if (action == 0 /*GLFW_RELEASE*/ && this.mc.currentScreen instanceof GuiControls)
               ((GuiControls)this.mc.currentScreen).buttonId = null; //Forge: Unset pure modifiers.
         }
